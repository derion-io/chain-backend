...
async function getBlockTimestamp(number) {
    ...
    if (nA >= safeHead) {
        if (warned['unsafe time anchor point'] != nA) {
            console.warn('unsafe time anchor point', { nA, head: configs.head, range })
            warned['unsafe time anchor point'] = nA
        }
    ...
    const interpolatedTimestamp = Math.floor(tA + ((tA - tB) / (nA - nB)) * (number - nA))
    const now = Math.floor(new Date().getTime() / 1000)
    return Math.min(interpolatedTimestamp, now)
}
...
async function _startProcessingLoop(context) {
    ...
    while (true) {
        // HEAD
        const depth = context.head - (context.lastHead ?? 0)
        const latency = context.head - (context.lastProcessed ?? 0)
